...
// If the user isn't already on the config page, include a link to it.
if (!onConfig)
{
	$("ul.nav.navbar-nav:contains('FORUM')").append("<li><a id='tsuminoEnhancedNavlink' target='_blank' href='/Enhanced' style='color:#22a7f0 !important;'>ENHANCED</a></li>");
}
...
if (onReader)
{
	if (GM_getValue("unstickedHeaderEnabled"))
	{
		tsuminoEnhanced.unstickyHeader();
	}
	if (GM_getValue("scrollToImageEnabled"))
	{
		tsuminoEnhanced.scrollToImage();
	}
	if(GM_getValue("slideshowEnabled"))
	{
